...
export default function MedicineCard({
  ...
  return (
    <article className="bg-white rounded-2xl sm:rounded-3xl p-5 sm:p-6 border border-slate-200 shadow-2xs hover:shadow-sm transition-all flex flex-col justify-between">
      ...
      <div>
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className="text-[17px] sm:text-[19px] font-extrabold text-slate-950 leading-snug">
                {displayName}
              </h3>
              {confirmed && (
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2">
              {(selectedCandidate?.short_composition || selectedCandidate?.genericName || medicine.composition) && (
                <p className="text-[12.5px] sm:text-[13px] text-slate-600 font-semibold flex items-center gap-1.5">
              ...
              {medicine.category && (
                <span className="text-[10.5px] sm:text-[11px] font-medium px-2 py-0.5 rounded-md bg-slate-100 text-slate-600">
                  {medicine.category}
                </span>
              )}
            </div>
          ...
          <div className="shrink-0">
            {medicine.confidence === "HIGH" ? (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11.5px] sm:text-[12px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 shadow-2xs">
                <CheckCircle2 size={13} />
                <span>Verified</span>
              </span>
            ) : medicine.confidence === "MEDIUM" ? (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11.5px] sm:text-[12px] font-bold bg-amber-50 text-amber-800 border border-amber-200 shadow-2xs">
                <AlertCircle size={13} />
                <span>Please verify</span>
              </span>
            ) : (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11.5px] sm:text-[12px] font-bold bg-rose-50 text-rose-700 border border-rose-200 shadow-2xs">
                <HelpCircle size={13} />
                <span>Unclear</span>
              </span>
            )}
        ...
        {/* Ambiguity / Candidate Confirmation Card */}
        {medicine.raw_text && (!confirmed || medicine.confidence !== "HIGH") && (
          <div className="mb-4 p-3 sm:p-3.5 rounded-xl bg-slate-50 border border-slate-200/90 text-[12px] sm:text-[12.5px] text-slate-700">
            <div className="flex items-center justify-between">
              <div>
                <span className="font-bold text-slate-900">Read as: </span>
                <span className="font-mono font-bold text-[#0284c7] bg-white px-2 py-0.5 rounded-md border border-slate-200 shadow-2xs">
                  &quot;{medicine.raw_text}&quot;
                </span>
              </div>
            </div>
            ...
            {medicine.candidate_matches && medicine.candidate_matches.length > 0 && !confirmed && (
              <div className="mt-3 pt-3 border-t border-slate-200 space-y-2">
                <p className="text-[11px] sm:text-[11.5px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1">
                ...
                {medicine.candidate_matches.map((cand, idx) => (
                  ...
                  >
                    <div className="min-w-0 pr-2">
                      <p className="text-[13px] sm:text-[13.5px] font-bold text-slate-950 truncate">
                        {cand.name}
                      </p>
                      <p className="text-[11px] sm:text-[11.5px] text-slate-500 truncate mt-0.5">
                        {cand.genericName || cand.short_composition} &bull; <span className="font-bold text-emerald-600">{Math.round(cand.similarity * 100)}% match</span>
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={() => handleConfirm(cand)}
                      className="shrink-0 px-3 py-1.5 rounded-lg bg-[#0f172a] hover:bg-[#1e293b] text-white text-[11.5px] sm:text-[12px] font-bold transition-all shadow-2xs cursor-pointer"
                    >
                      Use this
                    </button>
                  </div>
            ...
            {confirmed && (
              <div className="mt-2.5 flex items-center justify-between text-[11.5px] text-slate-500">
                <span>{useOriginal ? "Using exact handwritten text" : `Confirmed as ${selectedCandidate?.name}`}</span>
                <button
                  type="button"
        ...
        {/* 3-Part Daily Schedule Timeline (Morning, Afternoon, Night) */}
        {hasDosagePattern && (
          <div className="mb-3.5 p-3 rounded-xl bg-slate-50/80 border border-slate-100">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10.5px] sm:text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                Daily Dosage Schedule
              </span>
              {medicine.dosage.raw_text && (
                <span className="text-[11.5px] sm:text-[12px] font-mono font-bold text-slate-700 bg-white px-2 py-0.5 rounded border border-slate-200">
                  {medicine.dosage.raw_text}
                </span>
              )}
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              >
                ...
                <span className="text-[11px] sm:text-[11.5px]">Morning</span>
              </div>
              ...
              >
                ...
                <span className="text-[11px] sm:text-[11.5px]">Afternoon</span>
              </div>
              ...
              >
                ...
                <span className="text-[11px] sm:text-[11.5px]">Night</span>
              </div>
            </div>
          </div>
        )}

        {/* Structured Info (Timing & Duration) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-3.5">
          <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-100 flex items-center gap-2">
            ...
            <div className="min-w-0">
              <span className="text-[10px] sm:text-[10.5px] font-bold text-slate-400 uppercase tracking-wider block">Meal Timing</span>
              <span className="text-[12.5px] sm:text-[13px] font-bold text-slate-800 truncate block">
          ...
          <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-100 flex items-center gap-2">
            ...
            <div className="min-w-0">
              <span className="text-[10px] sm:text-[10.5px] font-bold text-slate-400 uppercase tracking-wider block">Duration</span>
              <span className="text-[12.5px] sm:text-[13px] font-bold text-slate-800 truncate block">
        ...
        {medicine.description && (
          <p className="text-[12.5px] sm:text-[13px] text-slate-600 leading-relaxed mb-3">
            {medicine.description}
          </p>
        )}
        ...
        {medicine.allergy_warning && (
          <div className="mb-2.5 p-3 rounded-xl bg-amber-50 border border-amber-200 flex items-start gap-2.5 text-[12px] sm:text-[12.5px] text-amber-950 font-medium">
        ...
        {medicine.completion_warning && (
          <div className="mb-2.5 p-3 rounded-xl bg-emerald-50 border border-emerald-200 flex items-start gap-2.5 text-[12px] sm:text-[12.5px] text-emerald-950 font-medium">
      ...
      <div className="pt-3 border-t border-slate-100 mt-2">
        <button
          ...
          className="inline-flex items-center gap-1 text-[11.5px] sm:text-[12px] font-semibold text-slate-500 hover:text-slate-900 transition-colors cursor-pointer"
        >
        ...
        {showEvidence && (
          <div className="mt-2.5 p-3 sm:p-3.5 rounded-xl bg-slate-50 border border-slate-200 text-[11.5px] sm:text-[12px] text-slate-700 space-y-1.5">
            <p className="font-bold text-slate-900">Neural Evidence &amp; Verification Consensus:</p>
            ...
            {medicine.manufacturer && (
              <p className="text-slate-500 pt-1.5 border-t border-slate-200 text-[11px]">
