...
export default function MedicineCard({
  ...
  return (
    <article className="bg-white rounded-3xl p-6 sm:p-7 border border-[#0D5C63]/10 shadow-premium flex flex-col justify-between transition-all hover:border-[#0D5C63]/25">
      ...
      <div>
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className="text-[19px] sm:text-[22px] font-extrabold text-[#004B49] leading-snug">
                {displayName}
              </h3>
              {confirmed && (
                <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-[#2D6A4F]/10 text-[#2D6A4F] border border-[#2D6A4F]/20">
                  <Check size={11} />
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-2">
              {(selectedCandidate?.short_composition || selectedCandidate?.genericName || medicine.composition) && (
                <p className="text-[13px] sm:text-[13.5px] text-slate-600 font-semibold flex items-center gap-1.5">
                  <Pill size={14} className="text-[#0D5C63] shrink-0" />
                  <span>{selectedCandidate?.short_composition || selectedCandidate?.genericName || medicine.composition}</span>
                </p>
              )}
              {medicine.category && (
                <span className="text-[11px] font-bold px-2 py-0.5 rounded-md bg-[#F9F6F0] text-[#0D5C63] border border-[#0D5C63]/15">
                  {medicine.category}
                </span>
              )}
            </div>
          ...
          <div className="shrink-0">
            {medicine.confidence === "HIGH" ? (
              <span className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full text-[12px] font-bold bg-[#2D6A4F]/10 text-[#2D6A4F] border border-[#2D6A4F]/20 shadow-2xs">
                <CheckCircle2 size={13} />
                <span>Verified</span>
              </span>
            ) : medicine.confidence === "MEDIUM" ? (
              <span className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full text-[12px] font-bold bg-amber-50 text-amber-800 border border-amber-200 shadow-2xs">
                <AlertCircle size={13} />
                <span>Please verify</span>
              </span>
            ) : (
              <span className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full text-[12px] font-bold bg-rose-50 text-rose-700 border border-rose-200 shadow-2xs">
                <HelpCircle size={13} />
                <span>Unclear</span>
              </span>
            )}
        ...
        {medicine.raw_text && (!confirmed || medicine.confidence !== "HIGH") && (
          <div className="mb-4 p-3.5 rounded-2xl bg-[#F9F6F0] border border-[#0D5C63]/15 text-[12.5px] text-slate-700">
            <div className="flex items-center justify-between">
              <div>
                <span className="font-bold text-slate-900">Read from prescription: </span>
                <span className="font-mono font-bold text-[#0D5C63] bg-white px-2.5 py-1 rounded-lg border border-slate-200 shadow-2xs">
                  &quot;{medicine.raw_text}&quot;
                </span>
              </div>
            </div>
            ...
            {medicine.candidate_matches && medicine.candidate_matches.length > 0 && !confirmed && (
              <div className="mt-3 pt-3 border-t border-[#0D5C63]/15 space-y-2">
                <p className="text-[11.5px] font-bold text-[#0D5C63] uppercase tracking-wider flex items-center gap-1">
                ...
                {medicine.candidate_matches.map((cand, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between gap-2 p-2.5 rounded-xl bg-white border border-slate-200 hover:border-[#0D5C63] hover:shadow-2xs transition-all"
                  >
                    ...
                    <button
                      ...
                      className="shrink-0 px-3.5 py-1.5 rounded-lg bg-[#0D5C63] hover:bg-[#004B49] text-white text-[12px] font-bold transition-all shadow-2xs cursor-pointer"
                    >
                      Use this
                    </button>
                  </div>
            ...
            {confirmed && (
              <div className="mt-2.5 flex items-center justify-between text-[11.5px] text-slate-500">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 font-bold text-[#0D5C63] hover:underline cursor-pointer"
                >
                  <RotateCcw size={11} />
        ...
        {hasDosagePattern && (
          <div className="mb-4 p-3.5 rounded-2xl bg-[#F9F6F0] border border-[#0D5C63]/10">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[11px] font-bold text-[#004B49] uppercase tracking-wider">
                Daily Dosage Schedule
              </span>
              {medicine.dosage.raw_text && (
                <span className="text-[12px] font-mono font-bold text-[#004B49] bg-white px-2.5 py-0.5 rounded-md border border-[#0D5C63]/20">
                  {medicine.dosage.raw_text}
                </span>
              )}
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              <div
                className={`py-2 px-1 rounded-xl text-center flex flex-col items-center gap-1 transition-all ${
                  isMorning
                    ...
                    : "bg-white border border-slate-200/80 text-slate-300 font-medium"
                }`}
              >
              ...
              <div
                className={`py-2 px-1 rounded-xl text-center flex flex-col items-center gap-1 transition-all ${
                  isNight
                    ? "bg-[#004B49] text-white font-bold shadow-xs"
                    : "bg-white border border-slate-200/80 text-slate-300 font-medium"
                }`}
              >
        ...
        {/* Structured Field Bento Cards: Meal Timing & Duration */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mb-4">
          <div className="p-3 rounded-2xl bg-[#F9F6F0] border border-[#0D5C63]/10 flex items-center gap-2.5">
            <Clock size={16} className="text-[#0D5C63] shrink-0" />
            <div className="min-w-0">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Meal Timing</span>
          ...
          <div className="p-3 rounded-2xl bg-[#F9F6F0] border border-[#0D5C63]/10 flex items-center gap-2.5">
            <Calendar size={16} className="text-[#0D5C63] shrink-0" />
            <div className="min-w-0">
        ...
        {medicine.allergy_warning && (
          <div className="mb-2.5 p-3 rounded-2xl bg-amber-50 border border-amber-200 flex items-start gap-2.5 text-[12.5px] text-amber-950 font-medium">
        ...
        {medicine.completion_warning && (
          <div className="mb-2.5 p-3 rounded-2xl bg-[#2D6A4F]/10 border border-[#2D6A4F]/20 flex items-start gap-2.5 text-[12.5px] text-[#2D6A4F] font-bold">
      ...
      {/* Expandable Verification Evidence Audit Drawer */}
      <div className="pt-3.5 border-t border-slate-100 mt-2">
        <button
          type="button"
          onClick={() => setShowEvidence(!showEvidence)}
          className="inline-flex items-center gap-1.5 text-[12px] font-bold text-[#0D5C63] hover:text-[#004B49] transition-colors cursor-pointer"
        >
          <Info size={13} className="text-[#0D5C63]" />
          <span>{showEvidence ? "Hide verification audit" : "View verification audit"}</span>
          {showEvidence ? <ChevronUp size={13} /> : <ChevronDown size={13} />}
        </button>

        {showEvidence && (
          <div className="mt-2.5 p-3.5 rounded-2xl bg-[#F9F6F0] border border-[#0D5C63]/15 text-[12px] text-slate-700 space-y-1.5">
            <p className="font-bold text-[#004B49]">Verification Consensus Log:</p>
            {medicine.confidence_reasons.map((r, i) => (
              <p key={i} className="flex items-start gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-[#0D5C63] shrink-0 mt-1.5" />
                <span>{r}</span>
              </p>
            ))}
